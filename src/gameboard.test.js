...
describe("Testing game board", () => {
    ...
    test("Spawn a ship", () => {
        ...
        expect(
            gameBoard.getPositionContents(2, 2).getShipRef()
        ).toBeInstanceOf(Ship);
        expect(
            gameBoard.getPositionContents(2, 3).getShipRef()
        ).toBeInstanceOf(Ship);
        expect(
            gameBoard.getPositionContents(0, 0).getShipRef()
        ).toBeInstanceOf(Ship);
        expect(
            gameBoard.getPositionContents(1, 0).getShipRef()
        ).toBeInstanceOf(Ship);
        expect(
            gameBoard.getPositionContents(2, 0).getShipRef()
        ).toBeInstanceOf(Ship);
        expect(
            gameBoard.getPositionContents(3, 0).getShipRef()
        ).toBeInstanceOf(Ship);
    ...
    test("Test ship position and reference on board", () => {
        ...
        expect(
            gameBoard.getPositionContents(2, 4).getShipRef()
        ).toBeInstanceOf(Ship);
    });
    test("Shoot the ship", () => {
        const gameBoard = new GameBoard();
        ...
        expect(gameBoard.getPositionContents(8, 8).wasAttacked()).toBe(
            true
        );
